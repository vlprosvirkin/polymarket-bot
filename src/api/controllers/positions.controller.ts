...
export class PositionsController {
    ...
    async getSummary(req: Request, res: Response) {
        try {
            const [openOrders, trades, balanceResponse] = await Promise.all([
                ...
                this.client.getTrades({ limit: 100 }),
                this.client.getBalanceAllowance({ asset_type: 'COLLATERAL' })
            ]);
     ...
     * Отменить все ордера
     */
    async cancelAllOrders(req: Request, res: Response) {
        try {
            const openOrders = await this.client.getOpenOrders();

            const results = await Promise.allSettled(
                openOrders.map(order => this.client.cancelOrder(order.id))
            );
