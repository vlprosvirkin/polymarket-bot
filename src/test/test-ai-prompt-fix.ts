...
async function testAIPromptFix() {
    console.log('🧪 Тест исправления AI промпта для recommendedAction\n');
    ...
    console.log('📊 Тестовый рынок:');
    console.log(`   Вопрос: ${testMarket.question.substring(0, 80)}...`);
    console.log(`   Market Price: YES ${((yesToken?.price || 0.93) * 100).toFixed(0)}% (ничего не случится)`);
    console.log(`   Market Price: NO ${((noToken?.price || 0.07) * 100).toFixed(0)}% (что-то случится)\n`);
    ...
    try {
        ...
        if (analysis.estimatedProbability !== undefined && yesToken) {
            ...
            console.log(`   AI Estimate (YES): ${(analysis.estimatedProbability * 100).toFixed(1)}%`);
            console.log(`   Market Price (YES): ${(marketPrice * 100).toFixed(1)}%`);
            console.log(`   Edge: ${(edge * 100).toFixed(1)} п.п.`);
            console.log(`   Recommended Action: ${analysis.recommendedAction || 'NONE'}`);
            ...
            if (analysis.recommendedAction === 'BUY_YES') {
                if (analysis.estimatedProbability > marketPrice) {
                    console.log(`   ✅ BUY_YES логичен: AI estimate (${(analysis.estimatedProbability * 100).toFixed(1)}%) > Market (${(marketPrice * 100).toFixed(1)}%)`);
                } else {
                    console.log(`   ❌ BUY_YES ОШИБКА: AI estimate (${(analysis.estimatedProbability * 100).toFixed(1)}%) <= Market (${(marketPrice * 100).toFixed(1)}%)`);
                    console.log(`   ⚠️  ДОЛЖНО БЫТЬ BUY_NO!`);
                }
            } else if (analysis.recommendedAction === 'BUY_NO') {
                if (analysis.estimatedProbability < marketPrice) {
                    console.log(`   ✅ BUY_NO логичен: AI estimate (${(analysis.estimatedProbability * 100).toFixed(1)}%) < Market (${(marketPrice * 100).toFixed(1)}%)`);
                } else {
                    console.log(`   ❌ BUY_NO ОШИБКА: AI estimate (${(analysis.estimatedProbability * 100).toFixed(1)}%) >= Market (${(marketPrice * 100).toFixed(1)}%)`);
                    console.log(`   ⚠️  ДОЛЖНО БЫТЬ BUY_YES!`);
                }
            } else {
                console.log(`   ℹ️  Action: ${analysis.recommendedAction || 'NONE'} - пропускаем проверку`);
            }
        ...
        if (hasError) {
            console.log('❌ ТЕСТ ПРОВАЛЕН: AI все еще делает логические ошибки в recommendedAction\n');
            process.exit(1);
        } else {
            console.log('✅ ТЕСТ ПРОЙДЕН: AI правильно определяет recommendedAction!\n');
            process.exit(0);
        }
