/**
 * Тест интеграции MarketFilter + PolymarketDataService
 * Демонстрирует фильтрацию обогащенных рынков с метриками ликвидности
 */

import { config as dotenvConfig } from "dotenv";
import { resolve } from "path";
import { ClobClient } from "@polymarket/clob-client";
import { PolymarketDataService, EnrichedMarket } from "../services/PolymarketDataService";
import { MarketFilter } from "../services/MarketFilter";

dotenvConfig({ path: resolve(__dirname, "../../.env") });

async function testEnrichedFilter() {
    console.log("╔════════════════════════════════════════════════════════════════╗");
    console.log("║     ТЕСТ ИНТЕГРАЦИИ: MarketFilter + PolymarketDataService     ║");
    console.log("╚════════════════════════════════════════════════════════════════╝\n");

    // Инициализация
    const client = new ClobClient("https://clob.polymarket.com", 137);
    const dataService = new PolymarketDataService(client);

    console.log("📡 Получение обогащенных рынков с orderbook и ликвидностью...\n");

    const enrichedMarkets = await dataService.getEnrichedMarkets({
        includeOrderbook: true,
        includeLiquidity: true
    });

    console.log(`✅ Получено ${enrichedMarkets.length} обогащенных рынков\n`);

    // Тест 1: Статистика по обогащенным рынкам
    console.log("═══════════════════════════════════════════════════════════════");
    console.log("1️⃣  СТАТИСТИКА ПО ОБОГАЩЕННЫМ РЫНКАМ");
    console.log("═══════════════════════════════════════════════════════════════");

    const stats = MarketFilter.getEnrichedMarketStats(enrichedMarkets);

    console.log(`   Всего рынков: ${stats.total}`);
    // avgVolume удален - volume не возвращается API
    console.log(`   Средняя вероятность: ${(stats.avgProbability * 100).toFixed(2)}%`);
    console.log(`   Средняя ликвидность: $${stats.avgLiquidity.toFixed(2)}`);
    console.log(`   Средний спред: ${stats.avgSpread.toFixed(2)}%`);
    console.log(`   С orderbook: ${stats.withOrderbook} (${((stats.withOrderbook / stats.total) * 100).toFixed(1)}%)`);
    console.log(`   С метриками ликвидности: ${stats.withLiquidity} (${((stats.withLiquidity / stats.total) * 100).toFixed(1)}%)`);
    console.log();

    // Тест 2: Фильтр по ликвидности
    console.log("═══════════════════════════════════════════════════════════════");
    console.log("2️⃣  ФИЛЬТР ПО ЛИКВИДНОСТИ");
    console.log("═══════════════════════════════════════════════════════════════");

    const liquidityThresholds = [50, 100, 200, 500];

    for (const threshold of liquidityThresholds) {
        const filtered = MarketFilter.filterEnrichedByLiquidity(enrichedMarkets, threshold);
        console.log(`   Ликвидность >= $${threshold}: ${filtered.length} рынков`);
    }
    console.log();

    // Тест 3: Фильтр по спреду
    console.log("═══════════════════════════════════════════════════════════════");
    console.log("3️⃣  ФИЛЬТР ПО СПРЕДУ (BID-ASK SPREAD)");
    console.log("═══════════════════════════════════════════════════════════════");

    const spreadThresholds = [1, 2, 5, 10];

    for (const threshold of spreadThresholds) {
        const filtered = MarketFilter.filterEnrichedBySpread(enrichedMarkets, threshold);
        console.log(`   Спред <= ${threshold}%: ${filtered.length} рынков`);
    }
    console.log();

    // Тест 4: Фильтр по глубине orderbook
    console.log("═══════════════════════════════════════════════════════════════");
    console.log("4️⃣  ФИЛЬТР ПО ГЛУБИНЕ ORDERBOOK");
    console.log("═══════════════════════════════════════════════════════════════");

    const depths = [3, 5, 10];

    for (const depth of depths) {
        const filtered = MarketFilter.filterEnrichedByOrderbookDepth(enrichedMarkets, depth, depth);
        console.log(`   Минимум ${depth} bid/ask: ${filtered.length} рынков`);
    }
    console.log();

    // Тест 5: Комплексная фильтрация для торговли
    console.log("═══════════════════════════════════════════════════════════════");
    console.log("5️⃣  КОМПЛЕКСНАЯ ФИЛЬТРАЦИЯ ДЛЯ ТОРГОВЛИ");
    console.log("═══════════════════════════════════════════════════════════════");

    const tradableMarkets = MarketFilter.filterEnrichedForTrading(
        enrichedMarkets,
        100,  // minLiquidity
        5,    // maxSpreadPercent
        3     // minOrderbookDepth
    );

    console.log(`   Конфигурация:`);
    console.log(`   - Минимальная ликвидность: $100`);
    console.log(`   - Максимальный спред: 5%`);
    console.log(`   - Минимальная глубина orderbook: 3 bid/ask`);
    console.log(`\n   ✅ Найдено ${tradableMarkets.length} ликвидных рынков для торговли\n`);

    if (tradableMarkets.length > 0) {
        console.log(`   📊 Топ-5 ликвидных рынков:\n`);

        tradableMarkets.slice(0, 5).forEach((market, i) => {
            const yesToken = market.tokens.find(t => t.outcome === 'Yes');
            const prob = yesToken ? (yesToken.price * 100).toFixed(2) : 'N/A';

            const liquidity = market.liquidityMetrics
                ? Math.min(market.liquidityMetrics.totalBidSize, market.liquidityMetrics.totalAskSize).toFixed(2)
                : 'N/A';

            const spread = market.liquidityMetrics
                ? market.liquidityMetrics.spreadPercent.toFixed(2)
                : 'N/A';

            const orderbookDepth = market.orderbook
                ? `${market.orderbook.bids.length}/${market.orderbook.asks.length}`
                : 'N/A';

            const question = market.question.length > 60
                ? market.question.substring(0, 60) + '...'
                : market.question;

            console.log(`   ${i + 1}. ${question}`);
            console.log(`      YES: ${prob}% | Liquidity: $${liquidity} | Spread: ${spread}% | Depth: ${orderbookDepth}`);
            console.log();
        });
    }

    // Тест 6: Pipeline фильтрации для Endgame стратегии
    console.log("═══════════════════════════════════════════════════════════════");
    console.log("6️⃣  PIPELINE ДЛЯ ENDGAME СТРАТЕГИИ");
    console.log("═══════════════════════════════════════════════════════════════");

    console.log(`   Шаг 1: Базовая фильтрация`);
    let filteredEnriched = MarketFilter.filterBasic(enrichedMarkets);
    console.log(`          → ${filteredEnriched.length} активных рынков`);

    console.log(`   Шаг 2: Фильтр по вероятности (90-99%)`);
    filteredEnriched = MarketFilter.filterByProbability(filteredEnriched, 0.90, 0.99);
    console.log(`          → ${filteredEnriched.length} рынков`);

    console.log(`   Шаг 3: Фильтр по дате разрешения (< 14 дней)`);
    filteredEnriched = MarketFilter.filterByResolutionDate(filteredEnriched, undefined, 14);
    console.log(`          → ${filteredEnriched.length} рынков`);

    console.log(`   Шаг 4: Фильтр по ликвидности (>$100, <5% spread)`);
    const filteredForTrading = MarketFilter.filterEnrichedForTrading(filteredEnriched, 100, 5, 3);
    console.log(`          → ${filteredForTrading.length} ликвидных рынков`);

    console.log(`   Шаг 5: Сортировка по вероятности`);
    // sortByVolume удален - volume не возвращается API. Используем сортировку по вероятности
    const sortedFiltered = MarketFilter.sortByProbability(filteredForTrading, true) as EnrichedMarket[];
    console.log(`          → Топ-10 рынков`);

    const top10: EnrichedMarket[] = sortedFiltered.slice(0, 10);

    console.log(`\n   ✅ Финальная выборка для Endgame: ${top10.length} рынков\n`);

    if (top10.length > 0) {
        console.log(`   📊 Результаты:\n`);

        top10.forEach((market, i) => {
            const yesToken = market.tokens.find(t => t.outcome === 'Yes');
            const prob = yesToken ? (yesToken.price * 100).toFixed(2) : 'N/A';
            const volume = parseFloat(market.volume || '0').toFixed(0);

            const liquidity = market.liquidityMetrics
                ? Math.min(market.liquidityMetrics.totalBidSize, market.liquidityMetrics.totalAskSize).toFixed(2)
                : 'N/A';

            const endDate = market.end_date_iso ? new Date(market.end_date_iso) : null;
            const daysToEnd = endDate
                ? ((endDate.getTime() - Date.now()) / (1000 * 60 * 60 * 24)).toFixed(1)
                : 'N/A';

            const question = market.question.length > 50
                ? market.question.substring(0, 50) + '...'
                : market.question;

            console.log(`   ${i + 1}. ${question}`);
            console.log(`      YES: ${prob}% | Volume: $${volume} | Liquidity: $${liquidity} | Days: ${daysToEnd}`);
        });
    }
    console.log();

    // Итоги
    console.log("═══════════════════════════════════════════════════════════════");
    console.log("✅ ИТОГИ ИНТЕГРАЦИИ");
    console.log("═══════════════════════════════════════════════════════════════");
    console.log(`✅ Всего обогащенных рынков: ${enrichedMarkets.length}`);
    console.log(`✅ С метриками ликвидности: ${stats.withLiquidity}`);
    console.log(`✅ Средняя ликвидность: $${stats.avgLiquidity.toFixed(2)}`);
    console.log(`✅ Средний спред: ${stats.avgSpread.toFixed(2)}%`);
    console.log(`✅ Ликвидных для торговли: ${tradableMarkets.length}`);
    console.log(`✅ Финальная Endgame выборка: ${top10.length}`);
    console.log("\n🎉 Интеграция MarketFilter + PolymarketDataService работает!\n");
}

// Запуск
testEnrichedFilter().catch(error => {
    console.error("❌ Ошибка:", error);
    process.exit(1);
});
