...
async function debug() {
    console.log("🔍 Диагностика фильтрации рынков\n");
    ...
    console.log(`1️⃣  Активные: ${step1.length}`);
    
    // Шаг 2: С токенами
    const step2 = step1.filter(m => m.tokens && m.tokens.length > 0);
    console.log(`2️⃣  С токенами: ${step2.length}`);
    ...
    console.log(`3️⃣  Объем >= $5000: ${step3.length}`);
    
    // Шаг 4: NegRisk
    const step4 = step3.filter(m => !m.neg_risk);
    console.log(`4️⃣  Без NegRisk: ${step4.length}`);
    ...
    console.log(`5️⃣  Цена 70-99%: ${step5.length}\n`);
    
    // Показываем где теряются рынки
    if (step5.length === 0) {
        console.log("❌ Проблема: все рынки отсеяны на шаге:");
        
        if (step4.length === 0 && step3.length > 0) {
            console.log("   → Шаг 4 (NegRisk) отсеял все");
            console.log(`   Примеры рынков с NegRisk:`);
            step3.slice(0, 3).forEach((m: any) => {
                console.log(`   - ${m.question.substring(0, 60)}... (negRisk: ${m.neg_risk})`);
            });
        } else if (step3.length === 0 && step2.length > 0) {
            console.log("   → Шаг 3 (Объем) отсеял все");
            console.log(`   Примеры объемов:`);
            step2.slice(0, 5).forEach((m: any) => {
                const vol = parseFloat(m.volume || "0");
                console.log(`   - ${m.question.substring(0, 50)}...: $${vol.toFixed(2)}`);
            });
        } else if (step5.length === 0 && step4.length > 0) {
            console.log("   → Шаг 5 (Цена) отсеял все");
            console.log(`   Примеры цен:`);
            step4.slice(0, 5).forEach((m: any) => {
                const yesToken = m.tokens.find((t: any) => t.outcome === 'Yes');
                const price = yesToken ? yesToken.price : 0;
                console.log(`   - ${m.question.substring(0, 50)}...: ${(price * 100).toFixed(1)}%`);
            });
        }
    } else {
        console.log(`✅ Найдено ${step5.length} подходящих рынков после базовой фильтрации`);
    }
}
